...
def cat_family():
    ...
    cats = []
    for i in range (5):
        cats.append(cc.Cat(names[i],dobs[i],male[i],intact[i]))
    return cats

@pytest.fixture
def breeding_cats():
    cat_found_family = [cc.Cat('Karen','1/1/2019',False,True),cc.Cat('Kelsie','1/1/2020',False,True),cc.Cat('George','1/1/2020',True,True)]
    return cat_found_family


def test_cat_creation(cat):
    assert cat.name == "Big_Tom"
    assert cat.age_in_yrs == 10

def test_add_single_cat(clowder, cat):
    clowder.cat_moves_in(cat)
    assert len(clowder.clowder) == 1
    assert clowder.get_clowder_names() == ["Big_Tom"]
